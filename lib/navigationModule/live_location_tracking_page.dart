...
class _LiveLocationTrackingPageState extends State<LiveLocationTrackingPage> with WidgetsBindingObserver {
  ...
  void dispose() {
    ...
    // Don't clear gesture callback to allow other pages to continue using it
    super.dispose();
  }
  ...
  void _handleGesture(GestureType gestureType) {
    print('🖐️ _handleGesture called with: $gestureType');
    if (!_gesturesEnabled) {
      print('❌ Gestures disabled, ignoring gesture');
      return;
    }
  ...
  void _navigateToNavigationPage() {
    print('🔄 Navigating to navigation page');
